...
class ProductRepository extends ServiceEntityRepository
{
    ...
    public function createListShop(ProductFilter $productFilter, ?Model $model = null): PaginationInterface
    {
        ...
        if ($productFilter->getMin() || $productFilter->getMax()) {
            if ('price' !== $productFilter->getSort()) {
                $query
                    ->join('p.productVariants', 'pv');
            }
            $query
        ...
        if ($productFilter->getMax()) {
            $query
                ->andWhere('pv.priceHT * (1 + t.rate) <= :max')
                ->setParameter('max', $productFilter->getMax());
        }

        if ($productFilter->getMin()) {
            $query
                ->andWhere('pv.priceHT * (1 + t.rate) >= :min')
                ->setParameter('min', $productFilter->getMin());
        }
